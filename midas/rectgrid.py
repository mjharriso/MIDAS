...
class quadmesh(object):
  ...
  def __init__(self,path=None,cyclic=False,tripolar_n=False,var=None,simple_grid=False,supergrid=None,lon=None,lat=None,is_latlon=True,is_cartesian=False,grid_type='generic',):
    ...
    -90.0 90.0
    >>> x=np.linspace(0.5,359.5,360);y=np.linspace(-89.5,89.5,180)
    >>> X,Y=np.meshgrid(x,y)
    ...
    if var is None and f is not None:
      ...
      if np.size(self.latq) > 1:
          ...
          self.latq=np.concatenate(([lat0],self.latq))      
  ...
  def geo_region(self,y=None,x=None,name=None):
     ...
     if xe>=xs:
         ...
         section['x_read'].append(np.arange(0,xe+1))
...
# data stored is not associated.
  ...
  def volume_integral(self,field=None,axis=None,normalize=True):
    ...
    >>> grid=quadmesh('http://data.nodc.noaa.gov/thredds/dodsC/woa/WOA09/NetCDFdata/temperature_annual_1deg.nc',var='t_an',cyclic=True)
    >>> IO = grid.geo_region(x=(30,120.),y=(-30.,25.))
    >>> S=state('http://data.nodc.noaa.gov/thredds/dodsC/woa/WOA09/NetCDFdata/temperature_monthly_1deg.nc',grid=grid,geo_region=IO,fields=['t_an'],default_calendar='noleap',time_indices=np.arange(0,1),verbose=False)
    >>> S.volume_integral('t_an','XYZ',normalize=False)
    >>> print sq(S.t_an_xyzint)
    1.31849709143e+16
    >>> S.volume_integral('t_an','XY',normalize=False)
    >>> print sq(np.sum(S.t_an_xyint,axis=1))
    1.31849709143e+16
    """
    ...
    if self.var_dict[field]['Z'] is not None:
        if hasattr(self.var_dict[field],'dz'):
            ...
            dz = np.ones((sout.shape[1],self.grid.jm,self.grid.im))      
  ...
  def time_avg(self,field=None,vol_weight=True,target=None):
    ...
    >>> S=state('http://data.nodc.noaa.gov/thredds/dodsC/woa/WOA09/NetCDFdata/temperature_monthly_1deg.nc',grid=grid,geo_region=IO,fields=['t_an'],default_calendar='noleap',time_indices=np.arange(0,12),verbose=False)
