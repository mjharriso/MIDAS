...
#        import vertmap_ALE
        ...
        print " mom6-coordinate not intalled \
                  build https://github.com/angus-g/mom6-coordinate \
                  into your current environment before starting the \
                  interpreter."
        return
...
#                xb2[k,:,:]=numpy.minimum(xb2[k-1,:,:]-1.e-9,xb2[k,:,:]) # avoid zero thicknesses
            ...
            if memstats:
                print 'Memory usage mom_remapping (pre): %s (kb)' % resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
#            vertmap_ALE.pyale_mod.remap(data,data2,xb1,xb2,method,bndy_extrapolation=bndy_extrapolation,missing=missing_value)
            ...
            cs.interpolation_scheme = imethod
            cs.degree = imethod
            data2=mom_remapping.remapping_core_h(h1,data,h2,cs)

            if memstats:
                print 'Memory usage mom_remapping (post): %s (kb)' % resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            data2=data2.T
            xb2=-xb2.T
